...
for v = -100E-9:10E-9:100E-9 % Loop on the tilt of PRM
    X = ['PRM_tilt=',num2str(v),'nrad'];
    disp(X);
    ...
    PRM = Add_Tilt(PRM,v); % Add a tilt in the vertical direction to the PRM
...
% Save table of data
T=table(PRM_tilt,sig8MHz_ref,sig6MHz_circ,sig56MHz_circ,sig56MHz_DP);
% T_quad=table(PRM_tilt,sig8MHz_ref_quad,sig6MHz_circ_quad,sig56MHz_circ_quad,sig56MHz_DP_quad);
writetable(T,'PD.txt');
