...
G1 = Grid(128,0.4);
PRM_RoC=[];
sig6MHz_circ_quad=[]; sig56MHz_circ_quad=[];
...
figure(v); E_Plot(AV_DR.Field_ref,'SB','SB_num',1) %trouver comment choisir le champ "circulating field"
vstring=int2str(v);
filename = ['6MHz_' vstring '_100-ref'];
saveas(gcf,filename);
figure(v+100); E_Plot(AV_DR.Field_ref,'SB','SB_num',2)
filename = ['56MHz_' vstring '_100-ref'];
saveas(gcf,filename);
figure(v+200); E_Plot(AV_DR.Field_ref,'carrier')
filename = ['Carrier_' vstring '_100-ref'];
title('Carrier')
saveas(gcf,filename);
